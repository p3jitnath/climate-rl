...
class RadiativeConvectiveModelEnv(gym.Env):
    ...
    def _get_state(self):
        ...
        state = np.array([emissivity, adj_lapse_rate], dtype=np.float32)
        return state

    def step(self, action):
        emissivity, adj_lapse_rate = action[0], action[1]

        emissivity = scipy.special.expit(emissivity)
    ...
    def _render_frame(self):
        ...
        ax1_bars = ax1.bar(ax1_labels, states, color=ax1_colors, width=0.75)
        ax1.set_ylim(0, 10)
